...
class Whisper:
    def __init__(
        ...
        assert size in VALID_MODEL_NAMES, (
            f"Invalid model size {size}. Valid sizes are {VALID_MODEL_NAMES}"
        )

        self._size = size
        self._default_lang = default_lang
        self._model_name = (
            size if default_lang != "en" else ENGLISH_MODEL_VERSION_BY_NAME[size]
        )
    ...
    def _transcribe(
        self, audio_path: str, prompt: str = None, language: str = None
    ) -> dict:
        assert any(audio_path.endswith(ext) for ext in [".wav", ".mp3", ".flac"]), (
            "Audio file must be in .wav, .mp3 or .flac format"
        )
        ...
        if prompt is None:
            ...
            result_prompt = self.model.transcribe(
                audio_path,
                word_timestamps=True,
                initial_prompt=prompt,
                language=language,
            )
            prompt_dist = edit_distance(
                prompt.strip().lower(), result_prompt["text"].strip().lower()
            )
            result_no_prompt = self.model.transcribe(
                audio_path, word_timestamps=True, language=language
            )
            no_prompt_dist = edit_distance(
                prompt.strip().lower(), result_no_prompt["text"].strip().lower()
            )
            result = result_prompt if prompt_dist < no_prompt_dist else result_no_prompt
        return result

    def check_audio_quality(
        self, audio_path: str, expected_text: str, language: str = None
    ) -> float:
        ...
        try:
            segments = self._adjust_segments_to_sentences(
                segments=segments, sentences=expected_sentences, force_reassignment=True
            )
        except AssertionError as e:
            logger.warning(
                f"Error adjusting segments to sentences. Probably there is a lost phrase. {e}"
            )
            return -1.0
        ...
        start_difference, end_difference = (
            abs(segments[0]["start"] - 0.0),
            abs(segments[-1]["end"] - audio_length),
        )
        ...
        distance = sum(
            edit_distance(
                expected_sentence.strip().lower(), segment["text"].strip().lower()
            )
        ...
        if text_penalty > 1.0:
            logger.warning(
                f"Text mismatch penalty is too high: {text_penalty:.2f}. "
        ...
        word_pace_by_segment = [
            (segment["end"] - segment["start"]) / len(segment["words"])
            for segment in segments
        ]
        # Calculate the standard deviation of the time/word
        std = math.sqrt(
            sum(
                (time - sum(word_pace_by_segment) / len(word_pace_by_segment)) ** 2
                for time in word_pace_by_segment
            )
            / len(word_pace_by_segment)
        )
        ...
        logger.debug(
            f"Quality score: {score:.2f} (Sentence Mismatch: {missmatch_penalty:.2f} "
    ...
    def audio_file_to_srt(
        self,
        audio_path: str,
        srt_sentence_output_path: str,
        ...
        if text_to_fit:
            ...
            segments = self._fit_text_to_segments(
                segments=segments, text_to_fit=text_to_fit
            )

        # Generate sentence-based SRT
        self._generate_sentence_srt(
            segments=segments, srt_file_path=srt_sentence_output_path
        )

        # Generate word-based SRT
        self._generate_word_srt(segments=segments, srt_file_path=srt_words_output_path)

    def _fit_text_to_segments(
        self, segments, text_to_fit: str, max_edit_distance: int = 3
    ):
        """
        ...
        if len(sentences) != len(segments):
            logger.warning(
                "Sentence count does not match segment count. Adjusting segments to match sentences."
            )
            logger.warning(
                f"Segments: {' - '.join([segment['text'] for segment in segments])}"
                f"\nSentences: {' - '.join(sentences)}"
            )
        segments = self._adjust_segments_to_sentences(
            segments=segments, sentences=sentences, force_reassignment=True
        )
        assert len(sentences) == len(segments), (
            "Sentence count does not match segment count"
        )
        ...
        for true_sentence, segment in zip(sentences, segments):
            ...
            for word_info in segment["words"]:
                ...
                window_end = min(
                    word_index + window_end_offset, len(words_from_sentences)
                )
                ...
                if best_match is not None:
                    ...
                    logger.warning(
                        f"Word mismatch: {predicted_word} not found in window."
                    )
    ...
    def _adjust_segments_to_sentences(
        self,
        segments: list[dict],
        sentences: list[str],
        ...
        def calculate_edit_distance(segment_text, sentence_text):
            return edit_distance(
                segment_text.strip().lower(), sentence_text.strip().lower()
            )
        ...
        for sentence in sentences[:-1]:
            ...
            for i in range(1, len(remaining_words) + 1):
                candidate_segment_text = "".join(
                    word["word"] for word in remaining_words[:i]
                ).strip()
                clean_segment_text = (
                    candidate_segment_text.translate(str.maketrans("", "", PUNCTUATION))
                    .strip()
                    .lower()
                )
                clean_sentence = (
                ...
                if len(
                    increment_dist_hist
                ) > max_words_increasing_to_cut_search + 1 and all(
                    prev_dist < next_dist
                    for prev_dist, next_dist in zip(
                        increment_dist_hist[:-1], increment_dist_hist[1:]
                    )
                ):
                    break
        ...
        assert len(remaining_words) > 0, (
            "Remaining words should not be empty, there is still one sentence left."
        )
    ...
    def _generate_sentence_srt(self, segments, srt_file_path):
        with open(srt_file_path, "w", encoding="utf-8") as f:
            for i, segment in enumerate(segments, start=1):
                ...
                f.write(f"{i}\n")
                f.write(
                    f"{self._format_time_srt(start_time)} --> {self._format_time_srt(end_time)}\n"
                )
                f.write(f"{text}\n\n")
