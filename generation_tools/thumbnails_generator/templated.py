...
class Templated:
    def __init__(self):
        ...
        assert all(
            os.getenv(api_key) is not None for api_key in TEMPLATES_BY_API_KEY.keys()
    ...
    def generate_thumbnail(self, text: str, image: str, output_path: str):
        ...
        if not api_key:
            raise ValueError(
                f"API key for '{api_key_name}' not found in environment variables"
            )
        image_url = self.url_generator.get_url_for_image(img_path=image)
        # Define the payload for the POST request
        payload = {
            "template": template_id,
            "layers": {"text": {"text": text}, "image": {"image_url": image_url}},
        }
        ...
        assert response.status_code == 200, (
            f"Failed to generate thumbnail: {response.text}"
        )
