...
from ai_content_pipeline.integrations.fanvue.auth import FanvueTokenManager
from fanvue_api_client.media import upload_media as upload_fanvue_media
from fanvue_api_client.posts import create_post as create_fanvue_post
...
class FanvueAPIPublisher:
    ...
    async def post_publication(
        ...
        logger.success("Post created: {}", post_data.get("uuid") or post_data.get("id"))
        return post_data
...
async def upload_media(file_path: Path, access_token: str) -> str:
    ...
    try:
        result = await upload_fanvue_media(
            upload_file,
            access_token,
            api_base_url=settings.fanvue_api_base_url,
        )

        if not result.success:
            raise Exception(f"Media upload failed: {result.error}")
        if not result.media_uuid:
            raise Exception("Media upload failed: missing media UUID")
...
async def create_post(
    ...
    Returns:
        Post data
    """
    return await create_fanvue_post(
        api_base_url=settings.fanvue_api_base_url,
        text=text,
        media_uuids=media_uuids,
        audience=audience,
