...
class Queue:
    ...
    def __init__(self) -> None:
        ...
        # A per-queue `queue.join()` is racy here: a queue that is still
        # empty when `run()` starts (e.g. generate_image, before `plan` has
        # fanned out into it) reports "drained" immediately and can never
        # un-report it once handlers enqueue into it later. One shared
        # counter across all queues avoids that race.
        self._pending = 0
