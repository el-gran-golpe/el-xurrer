...
async def test_run_waits_for_jobs_enqueued_by_a_handler_fan_out():
    ...
    async def generate_handler(batch: list[Job]) -> None:
        # A real suspension here, like the production backend's network
        # call, lets `run()` race ahead and cancel this worker mid-batch if
        # termination is (wrongly) based on a queue that looked empty
        # before the fan-out ever happened.
        await asyncio.sleep(0.01)
        generated.append(batch[0].id)

    async def plan_handler(batch: list[Job]) -> None:
        # A real suspension before fanning out, like the production
        # `await asyncio.to_thread(...)` planning call, is what exposes the
        # join()-per-queue race this test guards against.
        await asyncio.sleep(0.01)
        for i in range(3):
