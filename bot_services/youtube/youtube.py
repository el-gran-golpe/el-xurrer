...
class YoutubeAPI:
    def __init__(self):
        assert os.path.isfile(API_SECRETS_FILE), (
            f"API secrets file not found: {API_SECRETS_FILE}"
        )
        self.creds = None
        self._authenticate()
    ...
    def upload_video(
        ...
        assert title not in already_uploaded_videos, (
            f"Video with title '{title}' already exists."
        )
        ...
        logger.info(
            f"Video uploaded successfully: https://www.youtube.com/watch?v={response['id']}"
        )
        return response
