...
class BaseMain:
    ...
    def find_available_items(
        self, base_path, search_pattern=None, subdirs=None, item_type="items"
    ):
        """
    ...
    def prompt_user_selection(
        ...
        if not available_items:
            message = (
                not_found_message or f"No {item_type} found for {self.platform_name}."
            )
            print(message)
            return []
        ...
        try:
            if allow_multiple and user_input.lower() == "all":
                ...
                for index in indices:
                    assert 0 <= index < len(available_items), (
                        f"Invalid number: {index + 1}"
                    )
    ...
    def prompt_overwrite(self, existing_files, path_index=-1):
        ...
        overwrite_input = input(
            "Do you want to overwrite these existing files? (y/n): "
        )
